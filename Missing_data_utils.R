...
# cols_to_miss <- c("bmi", "sex")
# p<- 0.2
# dep_cols <- c("hyp", "age")
#
# df1<- create_missing(df, cols_to_miss,p=0.2, mech= "MCAR", dep_cols)
# naniar::vis_miss(df1[cols_to_miss])
...
create_missing <-
  function(df,
    ...
    # set random seed
    if (is.null(randomseed)) {randomseed= sample(1:1e9, 1)}
    n1<- dim(df[, cols_to_miss])[1]
    n2<- dim(df[, cols_to_miss])[2]

    # if not given, use all params for missing values
    if (mech == "MCAR") {
      {set.seed(randomseed);
        m1<- sample(1:(n1*n2), p*(n1*n2), replace= FALSE)}
      temp<- df[,cols_to_miss]
      temp[arrayInd(m1,.dim = c(n1,n2))] <- NA
      df[, cols_to_miss] <- temp
      remove(temp)
      #print(df[cols_to_miss])
      return(df)
    }

    if (mech == "MAR") {
      p1<- 0.9 #missing if lp < xxx
      p2<- 0.1  # missing if lp > xxx
      #pp - quantile for the linear predictor
      # %% missing will be p1*pp+p2*(1-pp) = p2 + pp(p1-p2) = p, hence
      pp <- (p - p2) / (p1-p2)
      # create linear predictor random numbers * dep_cols, and then
      # all the lp < XXX will have high chances of being missed (p1 =0.9)
      # change lp for each column to be missed
      for (c in cols_to_miss) {
        {set.seed(randomseed);
          lp <- apply(df[dep_cols] * rnorm(length(dep_cols)), 1, sum)
          }
        prob_miss <- ifelse(lp < quantile(lp, pp), p1, p2)
        {set.seed(randomseed+1);
          df[rbinom(n1, 1, prob_miss) == 1, c] <- NA
        }
      }
      return(df)
    }
    ...
    if (mech == "MNAR") {
      ...
      pp <- (p - p2) / (p1-p2)
      for (c in cols_to_miss) {
        prob_miss <- ifelse(df[,c] < quantile(df[,c], pp), p1, p2)
        {set.seed(randomseed);
          df[rbinom(n1, 1, prob_miss) == 1, c] <- NA}
      }
      return(df)
...
one_fold_run<-
  function(k, cv_folds, d1, d1_mice,d1_missForest){
    ...
    run_cc = TRUE
    if (dim(df_train_0)[1] < 150 |
        dim(df_test_0)[1] < 150 |
        sum(df_train_0$event == 1) < 10 |
        sum(df_test_0$event == 1) < 10) {
      run_cc = FALSE
    }
    ...
    df_train_3 <- impute_missforest(df_train_cv, params_impute,
                                    correctwealth = FALSE)$df
    ...
    getcindex <- function(df_train_x, df_test_x) {
      ...
      msrf<- mcox #survcompare::survsrf_train(df_train_x, params)
      pcox0 <- predict(mcox, df_test_x[params], type = "lp")
      plasso0 <-
        predict(mcoxlasso,as.matrix(df_test_x[params]),
                lambda = lambda.min,type = "link")
      psrf0 <- pcox0 #survcompare::survsrf_predict(msrf, df_test_x, fixed_time = 10)
      ...
      c0_srf <- c0_cox #concordancefit(y = y_test, x = 1- psrf0)$concordance
      remove(mcox, mcoxlasso, msrf)
      return(c(c0_cox, c0_lasso, c0_srf))
    }
    # CC
    if (run_cc) {
      cind_0 <- getcindex(df_train_0, df_test_0)
    } else{
      cind_0<- c(NaN, NaN, NaN)
    }
...
run_function <- function(X){
  ...
  d1<- create_missing(df,cols_to_miss, p=p, mech = mechi,dep_cols = dep_cols)
  trial1<-one_cv_run(d1, params, params, p, mechi,n = n)
  return(trial1)
}
...
postanalysis <- function(
  ...
  myrun_table_longer<-
    ...
    pivot_longer(cols = c(CoxPH,SRF, CoxLasso),
                 names_to = "model",
                 values_to = "C_index")
  myrun_table_longer <- data.frame(myrun_table_longer)
  means <-
    data.frame(
      myrun_table_longer %>%
  ...
  means[is.na(means)] = 0.5
  plt1<- means %>%
    ggplot(aes(x=p, y = Cindex, col = impute_name)) +
    facet_grid(cols = vars(mech), rows = vars(model)) +
    geom_line() # +
